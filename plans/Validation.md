...
1.1.112 — LDRT/STRT·LDRBT/STRBT가 privileged 권한을 그대로 사용하던 문제를 interpreter/x64/A64에서 수정했다. [ARM DDI0100I A4-48/60/197/206](https://e2e.ti.com/cfs-file/__key/communityserver-discussions-components-files/1023/ARM-Architecture.pdf)의 P=0,W=1 사용자 접근 규칙을 적용하며 register bank는 유지한다. 수정 전 interpreter16/fastmem24실패 재현; 기존 MPU145조건×3경로 통과. 즉시값/register offset·읽기전용·조건 불성립·cold/warm/권한 복구와 A64 생성 guard128조건(일반 post/pre-writeback 대조 포함), 관련 검사15개 통과. A64 helper 경계는 모델이며 native 장치 수락은 아니다. Black/Solatorobo 각1200 DS boot frames의 이전1.1.111 core 대조에서 화면·PCM 동일 및 ROM/save 보존. 단일쌍 출력 대조로 성능 개선을 주장하지 않는다. 같은 페이지 실행 권한 변경·실기 abort timing은 후속. 전체 suite 반복 없음.

1.1.113 — 하위 MPU region의 data/code 권한·cache/write-buffer 설정 갱신이 겹친 상위 region의 전체 속성을 덮어쓰던 문제 수정. [ARM946E-S DDI0201D 4.3](https://documentation-service.arm.com/static/5e8e3ee588295d1e18d3aa82)의 최고 번호 region 우선순위에 따라 가려진 주소 구간을 건너뛴다. 상위 region2/7, 실제 MCR7종, 허용 train→상위 쓰기 금지→복구42조건을 interpreter/JIT/fastmem에서 검사했다. 수정 전 각14개 deny 조건에서 금지된 쓰기가 실행됨을 재현했고, 수정 후42/42×3경로와 overlay 제거/상태 복원 통과. 관련18/18 검사, Black/Solatorobo 각1200 DS boot frames의1.1.112 core 대조에서 화면·PCM 동일 및 ROM/save 보존. 단일쌍으로 속도 향상을 주장하지 않는다. 절대 cache/abort timing·native ARM64·실기 수락은 별도이며 전체 suite 반복 없음.

1.1.114 — 같은 MPU 페이지에서 권한을 바꾼 뒤 static branch의 대상까지 JIT가 내장해 실행하던 문제와 cold trace의 Prefetch Abort 뒤 중복 예외로 SPSR이 덮이는 문제 수정. 권한/MPU/region MCR 또는 CPSR control MSR 뒤 taken branch에서는 대상의 기존 dispatch 권한 검사를 거치며 일반 분기 최적화와 순차 pipeline은 유지한다. Prefetch/Data Abort가 공통 exception notifier를 사용해 실패 trace 게시와 native fallback 후속 실행을 막는다. 수정 전 interpreter9/9, fastmem7/9: cold SPSR 손상과 warmed 금지 대상의 MOV/STR 실행을 재현했다. 확장18조건×3경로에서 MPU 활성화/region 제거/사용자 전환·복구·목적 메모리·실패 trace 미게시를 검증한다. [ARM DDI0100 A2.6.5](https://documentation-service.arm.com/static/5f8dacc8f86e16515cdb865a)의 fault address+4/LR와 CPSR 저장을 기준으로 하며 실기 절대 cycle·분기 없는 순차 prefetch 권한 처리는 후속이다.
관련 MPU·self-modifying 재진입·조건부 cycle 검사23/23 통과. Black/Solatorobo 각1200 DS boot frames의1.1.113 core 대조에서 화면·PCM 동일 및 ROM/save 보존. 단일쌍 출력 대조로 속도 개선을 주장하지 않으며, 전체 suite/동일 ARM64 emitter 검사는 반복하지 않았다. native ARM64 실제 실행은 별도 수락이다.


1.1.115 — Windows Software 화면의 Vulkan 표시를 기본 OFF 실험 옵션으로 추가했다. 기존 QPainter 합성을 BGRA 업로드로 표시하며 DS 3D/2D GPU 가속은 아직 없다. Qt 동적 로더를 사용해 Vulkan DLL을 필수 import로 추가하지 않는다. FIFO 표시, 프레임별 업로드 완료 fence와 image별 maintenance1 presentation fence로 재사용·해제를 관리한다. [Khronos semaphore reuse](https://docs.vulkan.org/guide/latest/swapchain_semaphore_reuse.html)의 presentation 자원 반납 규칙을 적용하며 fence 완료를 실제 화면 출력 시각으로 주장하지 않는다. 필요한 확장/format/driver가 없거나 표시가 실패하면 Native로 복귀한다.
실제 RTX 5080/Windows에서 3가지 크기·padded stride·36 frame·최소화/복원·destroy/recreate와 설정 Cancel/저장 검사2/2 통과. Khronos validation 및 synchronization validation 활성화 실행에서 VUID/동기화 오류 보고 없음. 실제 Black의 단색이 아닌 부팅 애니메이션에서 표시 픽셀 표본 전부 일치, paused resize와 Native 왕복 후 같은 frame 유지. 드라이버 목록이 없는 별도 프로세스에서 Native 게임 출력과 Vulkan 선호값 보존 확인. ROM/save hash 보존. 미지원 플랫폼 분기는 host compiler로 별도 컴파일했으며 native Linux/macOS 검증은 아니다. 다른 GPU/드라이버·장기 사용·driver hang·GPU rasterization·성능/물리 지연 수락은 후속이다. 전체 suite 반복 없음.


1.1.116 — Vulkan 3D 이식에 사용할 CPU edge setup·buffer 구조와 32개 shader variant 구성을 기존 GL 실행 경로에서 분리해 공유한다. GLSL은 동일한 DS 계산을 유지하며 Vulkan의 descriptor set0=storage, set1=MetaUniform, set2=sampler, set3=image 및 raster push constant offset0/8/16/20·depth offset0을 지정한다. 비선형 W 구간에서 쓰이지 않는 XRecip만 0으로 초기화해 저장 구조의 미정 값을 제거한다. 신규 Vulkan 게임 renderer 선택 기능은 아직 제공하지 않는다.
전체32개 native-resolution variant의 Vulkan1.1 SPIR-V 컴파일·spirv-val 통과. 실제 RTX5080 GL/Vulkan에서6개 폴리곤의 Z/W 각192구간,24word 전체와 출력 guard 일치. 선형/원근 보간·음수 texture 좌표·서로 다른 기울기를 포함하며 Khronos validation+sync validation 오류 보고 없음. 기존 GL texture/effect/workload/compile-failure 관련12/12 통과. Black/Solatorobo 각1200 frame의 기존 Compute 실행파일 대조에서 화면·PCM 동일과 ROM/save 보존. 첫 timing 차이 때문에 순서를 바꾼3쌍을 추가 비교했고 차이 부호와 baseline 자체 시간이 흔들려 성능 개선이나 무저하 보장은 주장하지 않는다. 전체 suite 반복 없음. 전체 Vulkan polygon binning→raster→blend/fog/AA→readback·게임 연결·다른 GPU/OS 수락은 후속이다.


1.1.117 — Minimum-phase sparse 응답의 나눗셈을 decoder event별 reciprocal로 캐시한다. 검증된 bank support 상한을 사용해 x64 tail 크기32byte를 유지하며 guest capture와 savestate 형식은 바꾸지 않는다. Black 동일 장면3쌍에서 frame 중앙값4.7~8.0% 감소, PCM·화면 동일. scalar 경로28조건의 최대 차이는4.01e-11 PCM 단위이며, 큰 절대 clock·혼합 period·tail 소멸과 DS/DSi rate/capture/rollback 검사가 통과했다. 다른 게임·CPU/OS의 같은 성능 향상을 보장하지 않는다.
기존 저버퍼 probe가 보간 index5를 거부하던 누락을 수정하고 장치 없는 PCM 검사에서도 보간을 선택할 수 있게 했다. 진단 모드의 max_processing_us는 source 읽기·대기와 볼륨·ramp·low-pass 처리를 포함하며 backend 변환/물리 장치 지연은 포함하지 않는다. 실제 Qt/Playback1/2에서 기존 Cosine/Cubic/Gaussian SDL128 callback과 Gaussian WASAPI 요청128→480을 각20초(steady16초) 실행해 공급 부족·유실0을 확인했다. 콜백 크기128이 실제 hardware period나2.67ms 물리 지연의 증거는 아니며 무음 제출 검사로 청감 틱 제거를 주장하지 않는다. Vulkan 3D 제품 통합은 별도 미릴리즈 작업이다.

1.1.118 — Vulkan 3D를 비디오 설정의 독립 renderer로 연결했다. native 256×192의 polygon binning→Z/W raster→depth/blend→fog/edge/AA를 GPU에서 실행하고 기존 Software 2D 합성·capture·RAM framebuffer 계약을 유지한다. 공통 texture cache의 일곱 DS 포맷/VRAM invalidation과 기존 GL의 polygon/meta/clear bitmap 준비 코드를 공유하며 32개 SPIR-V는 빌드에 내장한다. volk 동적 로딩으로 Vulkan driver 없는 PC에도 필수 DLL 의존성을 추가하지 않는다. 미지원/초기화 실패는 Software로, 프레임 실패는 frame 경계에서 Software로 복구한다. savestate 저장 때 현재 Vulkan 3D 화면을 지우던 통합 오류를 재현·수정했다.
RTX5080/Windows에서 native 화면33종·257 polygon 분할·Vulkan 3D→capture→실제 guest LDRH→texture→capture 검증 통과. 기존 pipeline의 GL/Vulkan 텍스처·효과·분할·동기화 검증과 Black/Solatorobo 각1200 frame의 Compute 대비 화면·PCM 동일/입력 파일 보존을 확인했다. 실제 Qt/DSi Black 상태에서 Vulkan→Software preview→Cancel→Vulkan 저장, Windows Black Vulkan 3D+Vulkan 표시의 창 크기 변경·Native 왕복·정지 frame 픽셀 표본 일치, 드라이버 없는 별도 프로세스의 Software/native 게임 출력 및 Vulkan 선호값 보존을 확인했다. Khronos validation+sync 오류 보고 없음. 기존 GL 관련12/12와 표시 복구11/11 통과; 표시 fixture의 누락된 기존 계측 멤버를 복원했다. 전체 suite 반복 없음.
GR-15 전체 완료는 아니다. 내부 고해상도·GPU 2D/capture·GPU image 직접 표시, AMD/Intel/native macOS/Linux/Android, 장기 사용·실제 device loss/driver hang 및 성능/물리 지연은 미검증 또는 후속이다. 현재 3D readback과 texture별 동기 업로드 비용이 있어 속도 향상을 주장하지 않으며 기존 renderer 기본값을 유지한다.
...
1.1.121 — 기존 Minimum-phase 옵션의 transition을10→8로 교체했다. 같은96dB 설계 목표에서 source/mixer 중 낮은 Nyquist의90% 지점 감쇠는 약-3.01→-0.61dB,95%는-20.41→-13.82dB다. 새 옵션/상태 형식 변경 없이 상단 고음을 보존하며, 내장 계수는 합계 약2.4MB 증가했다. 독립 r8brain/NumPy 재생성 두 bank의 바이트와 제품 loader 비교값이 모두 일치했고 기존1e-12 재생성 비교 기준은 유지했다. 새 고음 보존 회귀는 기존 bank에서 실패하고 교체 후 통과했다. 기존 note reuse·DS/DSi capture·rate/rollback·준비 실패 검증도 통과했다.
압축 step 최대 오차는 기존1.52e-7→후보3.45e-7로 증가했다. 조사 중 임의로 둔1e-9 step 기준은 양쪽 모두 실패했으며 이를 통과했다고 주장하지 않는다. 독립 raw 응답과 실제 stereo Push/Read의26조건(고정/가변 timer, full-scale gain/pan)의 후보 최대 차이는0.025 PCM 단위 미만, 정수 반올림 후 최대1단위였다. 전 입력/16채널 합산의 보편 오차 한계나 청감 무잡음 보장은 아니다.
동일 Black360frame·교차3쌍에서 화면 동일, frame 중앙값 비용 약7% 증가. DS/DSi16채널 가혹조건은 약18~24% 증가했고 guest SPU state는 동일했다. 실제 비무음 Playback1/2/WASAPI 요청128→480에서 공급 부족/폐기0. 두 논리 CPU/SDL128의 첫 실행은 빌드와 겹쳐2208샘플 부족24회가 발생했고 실패 기록을 보존했다. 빌드 종료 후 동일 조건에서 기존/후보 각16초를 비교해 양쪽 공급 부족·폐기·process-loopback 불연속0, steady 제출 PCM/loopback 일치를 확인했다. ROM/BIOS/state 원본 hash를 보존하고 쓰기 가능한 NAND는 매 실행 별도로 복사했다. 고부하 동시 작업에서의 공급 부족·장기 청취·native 저사양/다른 OS 수락과 추가 비용 최적화는 후속이며 전체 suite는 반복하지 않았다.


1.1.125 — 게임 전환·reset 시 SaveManager::SetPath가 FlushRequested를 세우던 것을 제거했다. 그 플래그는 CheckFlushLocked를 통해 이미 커밋된 SecondaryBuffer를 새 경로로 다시 쓰게 만들었고, 대상 경로는 새로 넣는 게임의 기존 save였으므로 이전 게임의 완료된 바이트가 그 save를 덮어쓸 수 있었다. 같은 크기여서 우연히 통과하는 경우와 달리 저장 크기가 다른 게임 사이에서는 길이까지 어긋난 파일이 남는다. 경로 이전은 이제 대기 중인 미커밋 바이트만 새 경로로 보내고 완료된 바이트는 다시 쓰지 않는다. UpdateConsole의 실제 순서는 flushSaveData로 기존 게임 바이트를 먼저 커밋한 뒤 SetPath를 부르므로 이전 자체에 커밋이 필요하지 않다.
검증: 신규 save-manager-relocation-clean 케이스는 이전을 완료된 상태에서 수행하고 대상 파일이 보존되는지, 매 프레임 도는 CheckFlush 뒤에도 보존되는지, 같은 매니저가 이후 새 게임 바이트를 새 경로에 정상 커밋하는지를 검사한다. 수정 전 코드에서 이 케이스가 실패하고(경로 이전만으로 대상 파일 재기록) 수정 후 통과하는 것을 확인했다. 기존 relocation-pending(미커밋 바이트 보존·명시 flush 시 새 경로 커밋)과 save-manager 전체18/18 PASS·실패0·skip0(7.83초)도 유지된다. 최종 Windows Ninja 빌드로 melonDS.exe까지 다시 링크했다.
한계: 결정론적 로컬 검증이며 실제 앱에서 두 게임을 오가며 겪는 전체 시나리오(실제 ROM·GBA·firmware save, 종료·복구 사본 경로, 다른 OS)는 이번 범위에 포함하지 않았다. 1.1.124의 자연 발생 재현율·점유자 귀속 한계도 그대로 남는다. 전체 CTest suite는 반복하지 않았다.

1.1.124 — Windows 저장 파일의 atomic replace가 순간 공유 위반으로 거부될 때만 유계 재시도하도록 SaveManager::WriteSaveFile을 바꿨다. 재시도는 Win32 오류 5/32/33(ERROR_ACCESS_DENIED/ERROR_SHARING_VIOLATION/ERROR_LOCK_VIOLATION)에서만 발동하고 기본5회·40ms 간격이며 MELONDS_SAVE_RETRY_ATTEMPTS(1~20)·MELONDS_SAVE_RETRY_DELAY_MS(10~5000)로 조정한다. 재시도 단위는 commit 실패한 QSaveFile을 다시 open하지 않고 매 시도 새 QSaveFile staging을 만든다. denied commit 뒤 객체를 재사용하면 빈 파일이 commit되므로 필수 조건이다. directWriteFallback=false는 그대로다.
검증: 최종 Windows Ninja 빌드에서 save-manager 집중 CTest 17/17 PASS·실패0·skip0(7.48초, 신규 save-manager-replace-retry 0.41초). 신규 케이스는 두 시나리오를 실제 Win32 핸들로 만든다. (1) 다른 프로세스가 FILE_SHARE_READ만으로 파일을 쥐고 120ms 뒤 놓으면 첫 atomic replace가 거부되고 bounded 재시도가 commit까지 도달한다. (2) 점유가 해제되지 않으면 재시도 예산 안에서 실패로 반환하고 pending·원본 바이트가 유지되며, 점유 해제 후 재flush가 성공해 새 바이트를 쓴다. 영구 거부 경로도3초 미만으로 종료돼 유계성을 확인했다. 기존 replace·retry-open·retry-rename·retry-worker·path-during-flush·buffer-resize·unpublished-pending·memory-copy-pending·flush-latest·recovery-copy·same-copy-path·copy-commit-failure·relocation-pending·allocation-capture·allocation-publish·producer-during-flush 회귀도 모두 통과했다.
한계: 이는 결정론적 lock fixture이며 1.1.82에서 관측한 자연 발생 실패의 재현율이나 실제 점유자 귀속을 증명하지 않는다. 그 진단은615회 중2회 실패했고 실패 직후 점유자는 포착되지 않아 트리거는 여전히 추정이다. 실제 디스크 부족·매체 제거·전원 장애, 백신/인덱서별 공유 동작, 실제 앱8조건 재수행, 다른 OS는 미검증이다. 전체 CTest suite는 반복하지 않았다.

1.1.122 — Vulkan 3D에 1x/2x/3x 내부 3D 해상도를 추가했다. scale별 32개씩 총 96개 compute shader를 생성·내장하고, 공유 ComputeData::PreparePolygon/PrepareMeta와 정수 연산을 그대로 재사용한다. 픽셀·타일·work·span 저장소, binning/final dispatch, readback extent, 배치 work 한도(indirect dispatch Z/X 실제 한도 포함)를 scale에서 파생한다. scale 변경은 새 pipeline과 그것을 참조하는 texture cache를 모두 만든 뒤 교체하며, 기존 native frame은 다음 RenderFrame까지 유지한다. clear color/depth VRAM 원본은 256×256, CPU 2D 합성·GetLine·guest capture 저장소는 256×192를 유지한다. 고해상도 결과는 native pixel origin을 선택하고 기존 RGB6/A5 변환만 적용하며 색·alpha를 평균해 새 값을 만들지 않는다. 1x는 hires 옵션이 켜져도 FinalPosition을 사용해 기존 DS 좌표 반올림을 보존한다.
검증: 최종 Windows Ninja 빌드(melonDS·VulkanRendererIntegration·VulkanComputeSpans·VulkanPresentation)와 Vulkan 집중 CTest 6/6 PASS·실패0·skip0(직접 재실행 6.76초). scale별 32개 native SPIR-V는 변경 전 산출물과 바이트 동일했다. shared-span Z/W 각 scale 192/384/576개 span의24 word와 output guard가 GL/Vulkan에서 일치했고, 전체 graph는 3scale×Z/W×8장면=48프레임에서 프레임당 49,152/196,608/442,368 픽셀 전부 일치했다. 분할 배치30건, scale별 33 native 화면 비교·257 polygon·capture→guest ARM LDRH→texture→capture 회귀, 1→2→3→1→3→2→1 라이브 전환의 frame 유지·hires coverage 변화·0/4 scale 거부·1x native rounding, missing-driver Software fallback과 GPU wait 실패 주입이 통과했다.
한계: 이는 구현과 제한된 로컬 검증이며 실제 기기·드라이버 수용이 아니다. 고해상도 dual-screen 표시/compositor를 추가한 것이 아니고 최종 화면·guest capture는 native resolve를 쓴다. 모든 2x/3x 장면이 1x Software와 완전 동일한 capture 픽셀이라는 강화된 의미의 exact identity는 증명하지 않는다. 실제 게임/DSi workload, 다양한 VRAM·부분 capture, GPU/driver matrix, Vulkan validation·synchronization validation, 실 UI의 4x~16x 복귀·오류 복구, 성능·peak VRAM·장시간 실행, 다른 OS/toolchain/배포 패키지는 미검증으로 남는다. 전체 CTest suite는 반복하지 않았다.

1.1.126 — GR-15 gate #4 실게임 등가성 sweep의 ROMSmoke readback 결함을 수정했다. scale>1에서 GL compute 경로의 front buffer는 256×ScaleFactor(2x 512×384, 3x 768×576)인데 하네스가 좌상단 256×192만 읽어 확대 프레임의 일부만 capture했다. Black compute 3x는 그 결과 전부 검정으로 읽혀 uniform-frame 가드가 exit 14로 오탐했고, 이는 제품 3D 결함이 아니라 하네스 결함이었다. ROMSmoke는 이제 front buffer 텍스처 크기를 GL_TEXTURE_WIDTH/HEIGHT로 조회해 256×192로 박스 평균하고 factor를 로그에 남기며, 크기가 256×192의 정수배가 아니면 exit 18로 실패한다. 진단용 MELONDS_SMOKE_SCALE/MELONDS_SMOKE_HIRES 훅을 추가하고 제품 기본값은 1x·hires off로 유지했다. 제품 소스는 바꾸지 않았다.
...
## 1.1.131~1.1.132 — 메인 체크아웃 재개 검증

2026-09-16. 1.1.131 캐시 제어/두-image 전송 코드는 기존 작업트리 전체962/962(실패0·skip0,102.92초)로 재검증했다.
이후 사용자 요청에 따라 깨끗한 `F:/melonDS`를 master로 전환하고, 이전 브랜치는 보존한 채 실제 메인 경로를 빌드했다.
메인 최초967개 중966개 통과, 저장상태 원자 교체1개 접근 거부가 있었다. 원본160바이트는 보존됐다.
단독 반복의 간헐 실패와 별도 첫-commit 잠금 해제 반례를 구분하여 보존했다. 외부 거부 주체는 확인하지 않았다.
1.1.132는 guest 직렬화1회, Windows RenameError만 최대5회/40ms 재시도, fresh QSaveFile 및 직접 덮어쓰기 금지를 적용한다.
정상·영구잠금·일시잠금20회씩(총60회)과 관련7개 검사가 통과했다. 최종 메인 전체는 **968/968, 실패0·skip0,122.15초**다.
등록 수 차이는 메인 CMake 구성과 새 반례1개로 발생하며,962+968을 독립 검사 개수로 합산하지 않는다.
실제 앱 도움말1.1.132, 메인 경로 `F:/melonDS/build/windows-dev/melonDS.exe`를 확인했다.
캐시 삭제 전후·자동 정리·cache 없이 출력하는 회귀는 GPU synthetic 검사이며 실기 oracle나 상용 게임 전체 검증은 아니다.
