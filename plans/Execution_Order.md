...
2026-09-13, 1.1.68: NP-18의 알려진512B/8KiB EEPROM을 생산 CartGame·Slot-2·실제 DMA에 연결했다. 패킷 길이로 용량을 추론하지 않으며 [GodMode9i 소비자](https://github.com/DS-Homebrew/GodMode9i/blob/8a8f806054133978e186d3a55d4058c551dde497/arm7/source/gba.c#L30)의 읽기·쓰기·작은 칩 긴 주소 판별10조건을 통과했다. 완성된8바이트만 저장에 반영하고 약6.5ms busy를 모델링한다. yield·단순 stall과 실제 종료·같은 CPU 선점을 구분하며 다른 CPU의 독립 DMA는 유지한다. 주소 증가/특정CPU·채널·18/6 설정만 허용하는 시제품 제한은 제거했다. 32비트 읽기 순서를 low→high로 명시하고, 128KiB 마지막 halfword와 무시되는 DMA 주소 하위 비트를 처리한다.

카트리지 reset/import·교체 때 이전 전송이 새 저장 버퍼에 이어지지 않으며, 실패한 할당/잘린 상태는 기존 칩·데이터·저장 통지를 보존한다. 정상 상태는14.2, 미완료 EEPROM/활성 전송은14.5로 저장한다. 전체 상태 저장 전에 형식을 정해 DS 카트리지 NC13 누락을 해결했다. 14.5는 구형 reader가 거부하며 생성13/14.2 카트 상태는 idle로 이관한다. 최종 Windows 전체841/842 후 기존 저장 commit 액세스 거부1개를 한 번 분리 재검증해 통과했고 최초 실패·종료코드8은 보존한다. 새17검사·SMC69개 및 JIT OFF17개 통과. 현행/원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.67과 같다. 초기 용량 판별·칩 변종·빠른 EXMEM의 전기적 오류·정밀 버스 중재·native ARM/실기와 FS-06 원인 추적은 남는다. GitHub Actions·macOS 빌드는 사용하지 않았다.

2026-09-13, 1.1.69: NP-18에서 기존 GBA 저장 파일이 없으면 SRAM/Flash 저장 메모리가 생성되지 않던 문제를 수정했다. [ares의 마커·용량 매핑](https://github.com/ares-emulator/ares/blob/af4cbb04f067682a8a3cf42695ff78bed634b38d/mia/medium/game-boy-advance.cpp#L118)을 대조해 완전한 세 자리 SDK 버전의 SRAM 32KiB·Flash 64/128KiB를 초기화한다. 기존 `.sav`의 전체 크기·내용과 RTC 꼬리 자료를 우선하며 최초 실제 쓰기 전에는 파일을 만들지 않는다. EEPROM 단독·서로 다른 종류/용량·불완전 마커는 용량을 추측하지 않는다. 세 자리 숫자가 아닌 homebrew 표기와 남아 있는 사용하지 않는 SDK 문자열은 이 fallback의 한계다.

생성 입력의 초기 판별5조건이 실패→통과했고 기존20개 저장 입력·불확실6조건을 함께 확인했다. 실제 Qt 삽입→Slot-2 쓰기→commit→재삽입5조건, 기존 EEPROM 우선·할당 실패의 이전 카트/저장 소유권 보존도 통과했다. 최초 비공개 드라이버의 잘못된 CPU 버스 소유권 선택을 EXMEMCNT로 바로잡았으며 제품 코드나 기대값을 우회하지 않았다. 최종 Windows 전체843/843(80.13초), 포함된 SMC69개 및 JIT OFF 관련8개 통과. 기존 FS-06의 간헐적 파일 교체 거부는 이번 실행에 없었지만 원인 미해결 상태를 유지한다.

현행/원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.68과 같고 개인 입력을 보존했다. 상태 형식을 추가하지 않았다. GitHub Actions·macOS 빌드는 사용하지 않았다.

다음 NP-18은 [MAME CC0 메타데이터](https://github.com/mamedev/mame/blob/1a326ed01c3629259abcf61a2a6bebf25858d376/hash/gba.xml)의 명시적 EEPROM 용량을 원본 전체 SHA-1·길이로 조회하는 작업이다. 정상 단일 ROM·고정 용량 항목만 재현 가능하게 변환하고 충돌·불완전 dump·미지정 용량은 제외한다. 기존 저장 파일을 우선하며 미일치/수정 ROM은 게임코드로 원본 용량을 강제하지 않는다. 명시적 카트 metadata 선택, 칩 변종·실물 timing·native ARM 및 나머지 공개 계획 수락은 별도로 남는다.

2026-09-13, 1.1.70: NP-18에 [고정 MAME 자료](https://github.com/mamedev/mame/blob/1a326ed01c3629259abcf61a2a6bebf25858d376/hash/gba.xml)의 전체 SHA-1·ROM 길이 기반 EEPROM 판별을 연결했다. 명시된512B 1,044개·8KiB 400개 식별값만 포함하며, 기존 파일이 없고 정확히 일치할 때 지워진 저장 메모리를 만든다. 기존 `.sav` 우선, 미일치 시 기존 SRAM/Flash 마커 fallback·EEPROM 용량 미확정 정책은 유지한다. borrowed/moved API 모두 실제 입력 길이로 해시하며 내부 ROM 패딩을 식별값에 섞지 않는다. 새 hash 라이브러리 없이 기존 SHA-1을 사용한다.

새 오프라인 생성기는 원본 XML 해시를 고정하고 정상 단일 ROM·동일 영역 길이·고정 용량만 변환한다. 용량 없는 일반 EEPROM3건·bad dump2건을 제외하고 동일2건을 합쳤다. 같은 해시에 길이/용량이 상충하면 출력 전에 거부한다. 생성 결과 재현과 합성 parser6그룹, 실제 source의 관련21검사·JIT OFF10검사를 통과했다. 별도의 생성 ROM 식별표로1.1.69의 초기 용량4실패를 재현한 뒤 두 용량의 첫 직렬 쓰기/읽기·상태·기존 파일 우선·수정/외부 패딩 ROM 거부를 확인했다. 이 fixture는 제품 식별표를 바꾸지 않으며 MAME 카탈로그 게임 전체 실행·실물 검증으로 세지 않는다.

최종 Windows 전체845/845(85.36초), 포함된 SMC69개가 통과했다. 현행/원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.69와 같고 개인 입력을 보존했다. Windows Git의 CRLF 체크아웃에서 생성기 `--check`가 실패하는 조건을 재현하고 생성 헤더의 LF 속성으로 바로잡았다. 이 속성만 바꾼 뒤 배포 소스 식별값을 다시 빌드했으며 core 바이너리 해시가 같아 실행 검증을 재사용했다. 간헐적 FS-06 파일 교체 거부는 이번에 발생하지 않았으나 원인 미해결이다. 상태 형식을 추가하지 않았으며 GitHub Actions·macOS 빌드는 사용하지 않았다.

다음 NP-18은 미등록/수정 카트리지에서 초기 저장 종류·용량을 명시하는 사용자 경로다. 기존 저장이 있으면 초기화/크기 변경으로 덮어쓰지 않으며, 취소·재선택·카트리지 교체 시 이전 선택을 다른 카트에 적용하지 않는다. DS 미등록 save 가정·chip 변종/정밀 timing·실기/native ARM 및 나머지 공개 계획 수락은 계속 남는다.

2026-09-13, 1.1.71: GBA 슬롯에 `Insert ROM cart with save type...`을 추가했다. 준비된 ROM의 삽입 요청마다 자동 판별 또는 EEPROM512B/8KiB·SRAM32KiB·Flash64/128KiB를 선택한다. 일반 삽입의 자동 판별은 유지하며 선택을 전역 설정이나 다음 카트리지에 남기지 않는다. 실제 로더는 허용 용량을 검증하고 기존 저장을 먼저 읽는다. 파일이 없을 때만 지워진 메모리를 만들고 첫 게임 쓰기에서 저장하며, 기존 내용·길이를 선택값으로 바꾸지 않는다. 빈 기존 파일에 수동 초기화를 요청하면 삽입을 거부한다. 선택창의 취소·닫기·ROM 재선택은 기존 준비 취소 토큰으로 연결한다.

기존 fixture에 추가한9그룹은 첫 실행에서9/9 통과했고 최종 Windows 전체854/854(94.72초), 포함된 SMC69개도 통과했다. 이전845개 검사 등록은 모두 유지했다. 다섯 용량의 첫 쓰기·재삽입, 기존 임의 길이·빈 파일·instance suffix, 잘못된 크기·취소·입력 불일치·할당 실패와 이전 소유권 보존을 확인했다. 실제 Qt 선택창과 ROM 준비의 메시지 전달 검증은 실제 앱 소비자 검증과 구분한다. DS↔DSi 설정 변경 시 새 메뉴의 활성 상태도 기존 GBA 메뉴와 함께 갱신한다. 최초 앱 빌드의 헤더 이름공간 오류는 수정했고 해당 실패 로그를 보존한다.

현행/원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.70과 같고 개인 입력을 보존했다. 새 상태 형식이나 게임 실행 중의 전역 선택 설정은 없다. 간헐적 FS-06 파일 교체 거부는 이번 전체 검사에서 없었으나 원인 미해결이며, 미등록 DS 초기 용량·칩 변종/WIP/WP·실기/native ARM·전체 콘솔 복원 실패 원자성 등은 남는다. GitHub Actions·macOS 빌드는 사용하지 않았다.

별도 실제 앱 객체로 QAction→파일 선택→ROM 준비→저장 종류 선택→에뮬레이션 스레드의 소비까지 연결해 첫 실행에서 통과했다. 다섯 크기의 첫 Slot-2 쓰기와 파일 전체 내용·상충 선택 후 재삽입, 기존 파일·취소·재선택·기본값 보존을 확인했다. 사용자 자료 경로 대신 비공개 임시 registry만 사용했고 선택·소비·저장 코드는 유지했다. Qt offscreen 생성 ROM 검사이며 상용 카트리지·물리 화면/오디오·DSi 부팅 검증으로 확대하지 않는다.

다음 NP-18은 NDS 미등록 ROM의8KiB 가정과 기존 저장 길이 경계다. `NDSCart.cpp`의 기본 SaveMemType과 `CartRetail` 생성자의 제한된 길이 복사를 실제 입력으로 재현하고, 기존 파일을 잘라 다시 저장할 수 있는 조건·일반 EEPROM/Flash 및 NAND의 구분을 확인한 뒤 판별/사용자 선택 경로를 보완한다.


2026-09-13, 1.1.72: 미등록 DS ROM의 기존512B/512KiB 저장이 첫 SPI 쓰기 후8KiB로 바뀌는 실제 앱 실패를 재현했다. 카트리지의 물리 칩 용량과 보존할 파일 길이를 분리해 큰 입력의 뒤쪽 데이터를 버리지 않는다. 등록 메타데이터는 그대로 사용하며, 미등록 게임만 기존 파일의 지원 SPI 용량512B·8/64/128KiB EEPROM과256/512KiB·1MiB Flash를 판별한다. 비표준 크기와8MiB는 예전8KiB 프로토콜을 유지하고 파일 전체를 보존하며, 파일 길이로 NAND를 선택하지 않는다. 알려진 칩보다 짧은 파일은 기존 바이트를 유지하면서 FF로 확장될 수 있다.

저장 callback에는 전체 파일 길이를 전달하고, 실제 변경 범위는 물리 칩 안에서 계산한다. 칩 끝을 넘거나 정확히 한 칩 전체를 쓰는 경우도 낮은 주소의 변경을 잃지 않는다. 상태는 기존14.2의 물리 영역만 기록하며, 다른 물리 용량을 불러올 때 SPI 주소 폭/프로토콜도 복원하고 수신 파일 여분을 유지한다. 비표준 용량/잘린 입력은 거부한다. 이 변경은 전체 콘솔 복원 실패의 원자성이나 파일 여분의 상태 백업 기능을 의미하지 않는다.

새 기존-target 회귀4그룹은 첫 실행4/4(1.85초), Windows 전체858/858(71.03초), JIT OFF의 관련 카트리지21/21이 통과했다. 이전854개 등록과 SMC69개를 유지했다. 현행 및 원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.71과 같고 개인 입력을 보존했다. 간헐적 FS-06 파일 교체 거부는 이번에 없었으나 원인 미해결이다. GitHub Actions·macOS 빌드는 실행하지 않았다.

별도 실제 앱의 동일 ROM/세이브 입력으로 수정 전2실패→수정 후2통과를 확인했다.512B와512KiB 모두 첫 저장·큐를 통한 재삽입 후 원래 길이와 요청한1바이트 외 모든 내용을 유지했다. 등록8KiB 메타데이터와512KiB 파일의 추가 조건도 통과했고, 먼저 저장 버퍼를 채운 뒤 칩 끝에서 주소0으로 넘어가는 쓰기가 파일 여분을 건드리지 않았다. 올바른 tiny EEPROM/Flash 칩 선택에 따라 SPI 주소 바이트/WRITE 명령은 바뀌며, 이를 동일 wire trace 실행으로 보고하지 않는다. 현재/frozen 입력1,093개와 호스트 registry 경로만 바꾼 복사본을 검증했다. Qt offscreen·생성 ROM·pause 후 실제 카트 SPI 검사이며 실물/상용 카트 수락으로 확대하지 않는다.

다음 NP-18은 저장 파일이 없거나 패딩/수정 ROM으로 자동 판별이 불확실할 때 DS 칩 종류·용량을 명시하는 사용자 경로다. 요청마다 선택을 전달하고 준비 취소·재선택·기존 바이트 보존을 확인한다. 기존 NAND의 ROM 명령/제조사 예약 영역을 SPI 선택과 혼동하지 않으며, SPI Flash8MiB·FRAM·WIP/WP·실기 timing·native ARM·전체 콘솔 복원 실패 원자성 및 다른 공개 계획의 미완료 범위를 유지한다.

2026-09-13, 1.1.73: DS의 `Open ROM with save type...`과 `Insert cart with save type...`을 추가했다. 자동·저장 없음·EEPROM512B/8/64/128KiB·Flash256/512KiB/1MiB를 해당 요청에만 전달한다. 명시적 선택은 등록 메타데이터도 덮어쓸 수 있지만 기존 파일 바이트와 여분을 유지한다. 빈/짧은 파일은 메모리에서 FF 확장하고 첫 게임 쓰기 전에는 파일을 바꾸지 않는다. NAND·homebrew·R4의 수동 SPI 선택과 범위 밖 값은 거부한다. 일반 열기·최근 목록·드롭은 자동이며 취소·재선택의 값이 후속 ROM에 남지 않는다. 상태 형식은 유지하고 상태 로드 시 그 상태의 칩 용량/프로토콜을 복원한다.

새 회귀9그룹은 첫 실행9/9(2.06초), Windows 전체는866/867(82.86초)이다. 기존 SMC69개는 통과했다. 유일한 전체 실패는 변경하지 않은 `save-manager-recovery-copy`의 Qt10 commit 액세스 거부이며, 단독1회는 통과했으나 FS-06 원인은 미해결이다. JIT OFF 관련21개 중20개가 먼저 통과했고, R4의 미사용 상태 태그를 기대한 테스트를 실제 클래스 검사로 수정한 뒤 영향1개가 통과했다. 제품 R4 상태는 바꾸지 않았고 최초 실패 로그를 유지한다. 현행/원본1.1 블랙600프레임×3 renderer의 마지막 화면과 개인 입력은1.1.72와 같다. GitHub Actions·macOS 빌드는 사용하지 않았다.

별도 실제 앱6그룹은 첫 실행6/6이며 메뉴·파일/종류 선택창·ROM 준비·실제 큐 소비의35흐름을 포함한다.9선택의 실제 칩 용량, 첫 SPI 쓰기/재로드,128KiB 칩+512KiB 파일의 wrap 후 여분 보존,512B/빈 파일의 Flash 확장과 취소·재선택을 확인했다. 현재/frozen 입력1,097개와 호스트 registry 경로만 격리한 복사본을 대조했다. Qt offscreen·생성 ROM·일시정지 후 실제 카트 SPI 범위이며 상용 게임/실기 수락으로 확대하지 않는다. private driver의 QMenu include 누락 컴파일 실패도 보존했다. 다음 NP-18은 DS Flash 쓰기/지우기와 EEPROM 페이지 경계를 chip 자료·실제 SPI 입력으로 확인하는 작업이다. WIP/WP·변종·실기/native ARM 및 FS-06 원인 등은 계속 남는다.

2026-09-13, 1.1.74: DS Flash가 PROGRAM 데이터를 모두0으로 쓰고, ERASE를 정렬되지 않은 주소부터0으로 채우며, 페이지 쓰기를 다음 페이지로 흘리던 문제를 수정했다. [Micron M25PE40 Rev.B](https://www.farnell.com/datasheets/2215260.pdf)의 p29/31/34/36에 따라256바이트 latch를 사용해 CS 해제에서 반영한다. PROGRAM은 비트1→0만 허용하고 WRITE는 마지막 수신 데이터를 사용한다. 페이지/섹터 지우기는 경계에 정렬한 영역을 FF로 만든다. WREN 없음·잘린 주소·지우기 뒤 여분 바이트는 저장하지 않는다. 짧은 쓰기의 정확한 변경 통지를 유지하며, 페이지 wrap은 해당 페이지를 통지해 파일 여분과 이전 SaveManager 캡처를 보존한다.

대기 중 Flash만 상태14.6을 요구하고256바이트 latch를 기존 기록의 표시 비트 뒤에 보존한다. 일반14.2·IR 파생 기록은 유지하며 전체 콘솔은 먼저 필요한 minor를 선택한다. 잘린/잘못된 대기 기록은 기존 저장 메모리·Flash latch·저장 통지를 바꾸기 전에 거부한다. 실제1.1.73 writer의 전송 중14.2를 새 reader로 이어서 읽었고,1.1.73 reader는 새 일반14.2를 읽고 대기14.6은 거부했다. 구형 상태에 이미 적용된 과거의 잘못된 쓰기를 되돌리는 기능은 아니다. 카트 공통/파생 상태를 포함한 전체 콘솔 실패 원자성도 별도다.

수정 전 Flash4그룹 실패→수정 후 통과. 관련28개 첫 실행26통과/IR2실패에서 과도한 통지 범위는 제품에서 수정하고, 조기 쓰기를 기대한 IR 상태 검사는 CS 해제 전 불변/14.6 조건으로 교체했다. 최종 영향8/8, 실제 DS/DSi ARM9/7·IR의13경로를 묶은1검사 첫 통과, JIT OFF19/19이다. 전체는872/873(206.29초)이며 변경하지 않은 `save-manager-unpublished-pending`에서 Qt10 commit 거부가 재발했다. 단독1회는 통과했으나 FS-06 원인은 미해결이다. SMC69개와 현행/원본1.1 블랙600프레임×3 renderer가 통과했고 마지막 화면은1.1.73과 같다. GitHub Actions·macOS 빌드는 사용하지 않았다.

다음 NP-18은 명시적인 EEPROM/FRAM 구분이다. [ST M95640](https://www.st.com/resource/en/datasheet/m95640-w.pdf)의 페이지 WRITE와 [FM25V10 FRAM](https://www.infineon.com/assets/row/public/documents/10/49/infineon-fm25v10-1-mbit-128-k-8-serial-spi-f-ram-datasheet-en.pdf)의 연속 WRITE가 다르므로 용량만으로 페이지 wrap을 일괄 적용하지 않는다. 검증된 카트 정보/명시적 선택에서 매체를 식별하고, 같은 용량의 상태 복원·변경 범위·기존 불명확 입력의 호환 동작을 함께 처리한다. Flash WIP/WP·칩 변종·실기/native ARM 수락은 남는다.

2026-09-13, 1.1.75: 명시적 EEPROM8/64/128KiB 선택에 각각32/128/256바이트 페이지를 적용하고 FRAM32KiB 선택을 추가했다. 페이지 규칙은 [M95640](https://www.st.com/resource/en/datasheet/m95640-w.pdf), [M95512](https://www.st.com/resource/en/datasheet/m95512-w.pdf), [M95M01](https://www.st.com/resource/en/datasheet/m95m01-r.pdf), 연속 쓰기는 [FM25W256](https://www.infineon.com/assets/row/public/documents/10/49/infineon-fm25w256-256-kbit-32k-x-8-serial-spi-f-ram-datasheet-en.pdf)을 기준으로 한다. EEPROM은 CS 해제에서 마지막 페이지 데이터를 반영하고 FRAM은 각 바이트를 즉시 반영하며 칩 끝에서0으로 돌아간다. 기존 자동 판별/코드2~4의 동작과 파일 여분은 유지하며,32KiB 파일이라는 이유만으로 FRAM을 추정하지 않는다.

명시적 매체는 idle에도 상태14.7을 사용하여 같은 용량의 기존 매체와 구분한다. 기존 일반14.2·Flash 대기14.6은 유지한다. 실제1.1.73 writer의14.2를 명시EEPROM 수신자에 읽으면 기존 연속 쓰기로 복원된다. 구형reader는 새 일반14.2를 읽지만 명시적 매체의14.7은 거부한다. 형식 헤더를 고쳐 호환성을 가장하지 않는다. 손상 프로필/용량/페이지 기록은 기존 저장 bytes·latch·통지를 바꾸기 전에 거부하며 전체 카트/콘솔 복원 실패 원자성은 별도다.
...
2026-09-13, 1.1.77: NP-18의 내부 WIP 지연에 앞서 NP-15 전송 순서를 수정했다. SPI 칩이 시작 즉시 바이트를 소비하고 완료 이벤트는 busy만 지우던 동작을, 예정된 완료 시점의 바이트 전달→자동CS 해제→busy 해제로 바꿨다. mode 해제·카트 교체·DSi 전원 차단은 미완료 바이트를 취소하며 기존 완료 이벤트는 유지한다. 전송 시작의 소유권 보존과 중도CS 취소는 실기 측정 전 에뮬레이터 정책이며, 전송 중 속도 변경·동시 소유 전환 전체를 해결한 것으로 세지 않는다.

새 미완료 바이트/CS 상태가 있을 때만14.9를 사용한다. 실제1.1.76 core·헤더를 보존한 writer와 현행 reader의 전체DS 상태40회에서, 이미 처리된 구형 바이트의 재실행 방지·새 바이트1회 완료·페이지 내용·idle 양방향 읽기·새14.9의 구형 명시 거부를 확인했다. 새 실제MMIO fixture는 이전 코어에서 실패했고 후보 SPI13/13 및 JIT OFF의SPI13+SMC23=36/36이 통과했다. DSi는 실제 frame scheduler와 전원 취소를 검사했으며 정확한DSi cold-state timing은 별도 수락이다.

사용자가 승인한 JIT SMC 검증도 최종 코어에서 실행했다. 기존69개(interpreter/JIT/fastmem각23)가 모두 통과했고, 별도 실제far 캐시4,079블록 생성→자동 리셋1회→SMC/이웃 보존91검사와 보존된1.1.59 interpreter24상태 대조에서 불일치0이다. JIT 제품 코드를 바꿔 검사 결과를 맞추지 않았다. native ARM64·실기 timing은 미검증이다.

Windows 전체 결과는883/884(86.11초)이며 savestate-file-roundtrip의QSaveFile commit이 액세스 거부로 실패했다. 해당 검사만1회 분리 실행해 통과했고, 최초 실패·종료코드8을 보존했다. 생성TEST payload를 저장하는 검사로 카트 전송을 실행하지 않으며 기존FS-06과 같은 파일 교체 거부 증상이나 동일 원인은 확정하지 않는다. 현행/원본1.1 블랙 상태600프레임×3 renderer의 마지막 화면은1.1.76과 같고 개인 입력은 보존했다. GitHub Actions·macOS 빌드는 사용하지 않았다.

다음 NP-18은 완료된SPI 전송 뒤 CS 상승에서 시작하는 칩 내부WIP 기한이다. EEPROM/Flash 실제 배열·저장 통지의 완료 시점, FRAM의즉시 쓰기, busy 중 명령과 reset/전원/상태 복원을 함께 처리한다. 물리WP·Flash 보호 변종·저장 보호 비트 영속성·NAND·실기/native ARM 및 나머지 공개 계획 수락은 계속 남는다.

2026-09-13, 1.1.78: NP-18의 알려진 EEPROM/Flash 내부WIP를 구현했다. SPI 완료→CS 상승→내부 쓰기→배열/저장 통지 순서이며, 상태 조회 없이도 완료하고 같은 시각의RDSR은 완료된 상태를 본다. 내부 latch는 다음SPI와 분리했고 구형 상태의 기존 배열·기한, DSi 전원1·sleep, reset/배출/가져오기/전원0 취소를 검증했다. FRAM과 용량만 아는 구형2/3/4는 기존 즉시 쓰기를 유지한다. 새 작업 상태만14.10이며 실제1.1.77/current 상태80회에서 허용·거부를 확인했다.

Windows 전체887/887(100.16초), SMC69(interpreter/JIT/fastmem각23), SPI14, JIT OFF44가 통과했다. 새 chip2그룹은 실제1.1.77 소스로 실패했다. 초기 잘못된 target명·SPI sleep helper 접근·RTC scheduler 추출 누락을 수정했으며, SPI 빌드 실패 후 잘못 실행한 구형SPI14개 결과는 전부 제외하고 현행 결과로 확인했다. 기존FS-06 접근 거부는 이번에 재현되지 않았으나 원인은 미해결이다. 블랙의 현행/원본1.1 상태600프레임×3 renderer 마지막 화면은1.1.77과 같고 개인 입력을 보존했다. GitHub Actions·macOS 빌드는 사용하지 않았다.

다음은 NP-18의Flash 보호 변종·매체 식별과 NP-15의소유권/전송 속도 경계 중 자료와 실제 반례를 확보할 수 있는 부분이다. 실기 측정·native ARM·전체 복원 실패 원자성과 나머지 계획은 계속 남긴다.


1.1.79 — NP-18 명시 M25PE T9HX 보호

DS 저장 종류에 M25PE20/40/80 T9HX(256/512/1024KiB)를 추가했다. BP/SRWD 상태 쓰기·64KiB별 write-lock/lock-down·JEDEC ID와 보호된 PP/PW/PE/SE 거부를 실제 슬롯/저장 흐름에 연결했다. 용량이 같아도 구형 변종과 보호 명령이 다르므로 기존 일반 Flash5/6/7을 유지한다. 새 명시 타입15/16/17은 idle도14.11 상태를 사용하며, 구형 reader는 이를 명시 거부한다. 기존 형식의 양방향 허용은 실제1.1.78/current 전체DS writer/reader82회로 확인했다.

Windows 전체는890/892(86.76초)였다. 두 실패는 새로 지원된15를 미지원 값으로 기대한 기존 검사이며, 미지원18로 수정하고 새15~17의 factory 허용/가족별 거부를 추가한 뒤 영향2/2가 통과했다. 최초 실패·종료코드8은 보존하고 전체를 반복하지 않았다. 새 chip3그룹은 실제1.1.78 소스에서 실패했으며 초기 fixture의 void* 산술 컴파일 오류도 보존했다. JIT OFF48, SPI15, SMC69 및 실제far 캐시4,079블록→자동 리셋1회→SMC91검사가 통과했다. 보존된 interpreter24상태와 불일치0이다. 블랙의 현행/원본1.1 상태600프레임×3 renderer 최종 화면은1.1.78과 같고 개인 입력을 보존했다. 마지막 수정은 테스트와 배포 소스 식별값이며 core·호환 입력·게임 binary 해시가 같아 성공 결과를 재사용했다.

새 Qt 선택/큐 검사는 추출 메서드와 실제 로더/MMIO 소비 검사를 구분하며, 이번 버전의 전체 앱 E2E로 세지 않는다. 다음은 NP-18의 명시 칩별 추가 erase/power-down 명령·매체 판별과 NP-15의 소유권/전송 속도 경계다. W#/RESET# 핀·보호 비트 재삽입 영속성·NAND·전체 복원 실패 원자성·native ARM/실기 timing과 나머지 계획은 남는다. 기존FS-06 파일 교체 거부의 원인도 미해결이다. GitHub Actions·macOS 빌드는 사용하지 않았다.

1.1.80 — NP-18 명시 T9HX 추가 지우기

M25PE20/40/80 T9HX의20 명령(4KiB)·C7 명령(물리 chip 전체)을 구현했다. WREN·정확한 명령 길이·CS 상승에서 수락하고 내부 완료 때 실제 배열과 저장 파일 변경을 게시한다. BP나 어느 sector의WL도 전체 지우기를 차단하며 LD만 있는 경우는 허용한다. 파일 끝의 여분 바이트와 일반Flash5/6/7은 유지한다. 지연은ST 자료의 전형값40ms·4.5/5/10초를 사용하며 실기 측정값은 아니다.

최종 Windows 전체896/896(225.09초), JIT OFF51/51, SMC69/69 통과. 새 chip2그룹은 실제1.1.79 소스에서 실패했고, 실제MMIO10경로·SaveManager 파일/재로드 및 이전/현행 전체DS 상태158회를 확인했다. 새 opcode가 controller에 남은 순간부터 진행 중에는14.12를 요구해1.1.79가 명시 거부하고, 완료 후14.11은 읽는다. 블랙의 현행/원본1.1 상태 각각600프레임×3 renderer 최종 화면은1.1.79와 같으며 개인 입력·연결한 core를 보존했다. 이전far-cache stress를 이번 버전 재실행으로 세지 않는다.

다음은B9/AB 절전의 독립적인 power phase·기한과 raw save 가져오기 경계다. 현재 가져오기는SaveEvent를 취소하므로 절전 진입/복귀를 WIP와 합치면 안 된다. 절전 자료 확인은 끝났지만 구현은 아직 없다. 물리 핀·전원 손상·자동 매체 판별·NAND·전체 복원 실패 원자성·NP-15 소유권/속도 경계·FS-06 원인·native ARM/실기와 나머지 공개 계획 수락은 남는다. GitHub Actions·macOS 빌드는 사용하지 않았다.

1.1.81 — NP-18 명시 T9HX 절전·복귀

B9/AB의진입·수면·복귀와CS 차단을 내부WIP와 분리했다. 기존scheduler 이벤트를 함께 쓰되 동시에 쓰기/절전 작업을 허용하지 않으며, raw save 가져오기는쓰기만 취소하고 원래power 기한을 유지한다. 전환 중 시작한SPI는 기한이 지난 뒤 첫 바이트가 도착해도 끝까지 무시한다. 전원0/reset은깨우고, 절전만으로 배열/저장 통지를 바꾸지 않는다. ST의3us/30us 최대값과 조기CS 거부는 결정적 모델이며 실물 측정값은 아니다.

절전/복귀·차단CS·대기B9/AB만14.13이 필요하고 awake 일반 명시칩 상태는14.11이다. 실제1.1.80/current 전체DS 상태280회에서 기존형식·완료 상태의허용과 새형식 거부를 확인했다. 구코어가 이미 전달받아 무시한B9/AB latch는재생하지 않지만, 아직전송 중이던B9는 새코어에서바이트 완료 후처음 실행한다. chip2그룹은보존한1.1.80에서실패했으며 손상payload 검증도추가했다.

전체898/899(200.06초)의실패는변경하지 않은standalone SaveManagerIO의Qt10 접근 거부다. 같은소스/assertion으로단독1/1 통과했으나FS-06 원인은미해결이며 전체를반복하지 않았다. JIT OFF54·SPI17·SMC69와DS/DSi/IR5경로, 블랙현행/원본1.1 상태600프레임×3의최종 화면·개인 입력보존은통과했다. 새Qt 전체앱/실제저장파일E2E나물리 timing 수락은아니다.

다음은재발한P1 FS-06의공유/파일교체 경계를생성파일로계측해원인을격리하는 작업이다. NP-15 소유권/중도전송속도의물리 계약도따로확인하며TODO만으로추측구현하지 않는다. NP-18 매체식별·NAND·물리핀/전원손상·보호영속성·전체복원실패원자성·native ARM/실기와나머지공개계획은남는다. GitHub Actions·macOS 빌드는사용하지 않았다.

NP-15 후속 정적 후보: 전송 중 소유권을 넘긴 뒤 이전 CPU가 SPI mode를 끄면 WriteSPICnt는 SPIToCart를 유지하지만 Interface::DoSavestate는 mode=0인 같은 상태를 거부한다. 두 경로의 정적 불일치를 확인했으며 실행 재현은 아직 없다. 기존 CartSPI의 TestByteCompletion에서 재현한 뒤 현재 정책/상태 계약을 수정한다. 중간 속도 변경의 물리 위상은 별개다: Hold=0·Busy=1에서 분주값3→0 전환 시 첫 적용SCK, 기존 위상 유지 여부, 마지막SCK/CS/Busy 시점을 실측해야 한다.

2026-09-13, 1.1.82: NP-15의 전송 중 소유권 변경→이전 CPU의 mode 해제에서 자체 전체 상태 저장이 실패하는 문제를 ARM9/7에서 재현하고 수정했다. 시작한 CPU의 취소는 busy/deadline을 유지하며 바이트 전달을 폐기하고, 소유권을 잃은 selection을 분리한다. 다른 CPU가 이미 선택한 거래는 조기 해제하지 않는다. 초기 후보 검토에서 held 완료 후 mode 먼저 복구→재소유 시 선택 누락과 경쟁 held WREN의 조기 확정을 추가 재현해 함께 수정했다. 이는 기존 소유권/취소 모델의 일관성 보완이며 실기 중재 규칙을 측정한 결과는 아니다.

최종 Windows900/900(100.93초), JIT OFF55/55, SMC69·SPI18과 실제1.1.81/current 전체DS 상태36회가 통과했다. pending 형식14.9를 유지하며 구 reader가 새 취소 상태를 읽는다. 현행/원본1.1 블랙600프레임×3의 최종 화면·개인 입력을 보존했다. 초기 후보의 전체 성공도 보존했고 검토에서 드러난 실제 결함에 따른 코드 변경 후 최종 검사를 실행했다. 전체 동시 인터페이스 중재·DSi cold 소유권 경계·실기 속도/CS·전체 복원 실패 원자성과 나머지 계획은 남는다.

FS-06은 생성 파일615회(의도적 자체 점유 대조3·비제어612)에서 Win32 및 직접NT rename 각각1회가 접근 거부를 반환했다. NTSTATUS/IO status는C0000022였고 기존 파일의 전체 bytes는 보존됐다. 28~36us 뒤 DELETE 접근은 성공했으며 이후 Restart Manager 점유자는0건이었다. 자체 점유 대조는 공유 오류32와 자기 PID를 찾았지만, 사후 관측은 순간 점유/필터를 배제하지 못한다. Qt의비제어256회 성공과이번 전체 검사의 성공은 FS-06 해결 증거가 아니다. 다음은 생성 파일에 한정한 실패 시점의 원인 귀속이며, NP-15 동시 전송 계약과 NP-18 매체/NAND·실기/native ARM 등 공개 잔여 계획도 이어간다. GitHub Actions·macOS 빌드는 사용하지 않았다.

1.1.83은 AD-04의 수동 저버퍼·설정 적용/복구 흐름을 하나로 묶는다. 다음 큰 오디오 묶음은 실제 DS 채널을 보존하는 SpeexDSP Q3 비교와 WASAPI 공유 저지연 출력 경로다. 타임스트레치/상한256·512의 선택형자동버퍼/ASIO는 옵션별 지연과 장치 수락을 따로 검증한다. 생성 NAND import 뒤 이전page재기록 재현은 NP-18에 보존해 뒤이어 수정한다.

1.1.84에서 WASAPI 공유 출력·장치 선택·tuple 복구를 통합했다. 다음 오디오 작업은 Q3 관측 프로토타입에서 드러난 가변 timer의 실제 시각/출력 전달과 채널 정렬 문제를 해결하는 비교다. 고정 timer 음질 성공을 일반 보간 완료로 바꾸지 않는다. 타임스트레치·상한 자동버퍼·ASIO 및 NP-18 import 뒤 stale page 재기록은 계속 남아 있다.

1.1.85는 AD-04의 선택형 음높이 유지 배속을 Rubber Band 4.0.0 R3로 구현한다. 기본값은 꺼짐이며 처리 지연을 UI에 표시한다. 생산 스레드가 변환하고 callback은 유한 PCM 큐만 읽는다. 속도 변경·큐 포화·동시 소비·성공한 상태 로드의 이력 초기화·실패한 로드의 보존 및 설정 미리보기/취소를 검증했다. 실제 게임의 0.5/1/2배속 360프레임씩에서 화면 동일성과 코어 출력 폐기0을 확인했지만 시작 대기와 짧은 공급 부족은 남았다. 청취·물리 지연·ARM·장기 실행·달성 불가능한 배속은 미검증이다. 고품질 보간 AD-08은 별도 미완료: Q3의 전 timer 범위와 짧은 고정 지연을 동시에 만족하지 못했고, event-blip은 저속 source image 억제가 Cubic보다 나빠 채택하지 않았다. 다음은 이 실패 조건을 만족할 보간 방식의 제한된 비교이며 자동버퍼·ASIO와 NP-18도 유지한다.


1.1.86은 NP-18의 카트리지 raw save 가져오기 뒤 대기 NAND 페이지가 새 데이터를 덮던 결함을 수정한다. 기본 SPI 쓰기 취소에 대응해 NAND 주소·쓰기 허용·페이지 이력을 비운다. 일반 pending commit과 savestate의 pending 복원은 유지한다. 실제 MMIO/scheduler·생성 NAND의 가져오기/live/cold 및 정상 commit/cold·실패 load 복구5조건이 통과했다. 이는 전체 NAND 프로토콜·실기 수락의 완료가 아니다. AD-08은 최소위상 FIR primitive의 고정 timer 직접 처리에서 선택 잡음 억제와 짧은 필터 지연을 확인했으며, 다음은 가변 timer/이력·cutoff 변경과 전 범위 비용이다. 원본 r8brain 블록 API의 대기와 필터 자체 지연을 구분하고 제품 보간은 아직 바꾸지 않는다.


1.1.87은 AD-04의 WASAPI 시작·일시정지 경계를 수정한다. 장치 사전 검증 이후 공유 스트림을 무음으로 유지하고, 실행/재개 때 에뮬레이터 callback 전달만 연다. Stop은 진행 중인 client callback이 끝날 때까지 기다리고 Close는 장치를 해제한다. 장치 버퍼 확대나 생산 PCM 폐기는 추가하지 않는다. Playback 1/2의 블랙360프레임·100ms 일시정지3회 비교에서 요청128/256/512의 전체 공급 부족은 기존5281/5761/7233에서321/2027/1025프레임으로 줄었다. 각 재개 구간의 첫 callback 이후20ms 부족은 최종9구간 모두0이었다. 시작·정상 실행 부족과 청취·실제Qt설정창·물리 지연·장치 상실 수락은 남는다. 다음은 남은 공급 시점/파형 불연속의 분리이며 AD-08 고품질 보간은 아직 별도 준비 코드다.


1.1.88은 AD-04의 제어 요청에 의해 중단된 오디오 대기를 다음 생산 프레임 전에 마친다. 설정창 일시정지가 큐가 찬 시점에 대기를 취소하면 기존 재개는 새 프레임부터 생산해 PCM을 폐기할 수 있었다. 취소된 대기만 다시 확인하고 추가 제어 요청은 계속 처리한다. 기존 큐 크기·대기 임계값·배속 정책은 유지한다. 실제Qt EmuThread/에뮬레이션 설정창·Playback 1/2의 블랙 상태 복원 뒤128/512 각각3회 열기/닫기에서 후보는 폐기0, 첫 설정창 이후 추가 공급 부족0이었다. 이전128 실행의 폐기7과 구분하며 장기 무잡음·물리 지연 수락으로 확대하지 않는다. offscreen 표시·출력 음량0 조건이다. AD-08 보간·실행 중/시작 틱·파형 불연속의 나머지 원인·다른 backend/ARM 수락은 남는다.


1.1.89는 AD-04의 일시정지/재개 전환을 DSP 이후 최종 장치 출력에 둔다. SDL·WASAPI 모두 pause에서 source callback을 중단하고 마지막 제출 sample부터 약1ms에 걸쳐 무음으로 전환하며, resume도 최종 출력에서 연결한다. 필터 앞의 중복 resume fade는 제거하고 shortage history만 초기화한다. 정상 공급 PCM은 전환 이후 그대로이며 추가 큐·PCM 폐기를 넣지 않는다. paused tail 이후는 표준 zero-fill로 처리한다. 기존 Qt 설정창/실제 Playback 1/2의 양 backend×128/512는 PCM 폐기0이며 SDL128에는 첫 설정창 이후219프레임 부족이 남았다. 실제 청취·물리 지연·장치 상실·ARM 및 AD-08 고품질 보간은 계속 남는다.


1.1.90은 오디오 설정의 출력 방법을 runtime 장치 조회로 구성한다. WASAPI 초기화/열거 실패 또는 endpoint 없음은 선택 가능한 목록에서 제외하고 저장된 선택이면 사용 불가로 보존한다. 사라진 장치에 대한 Preview/OK 적용을 막아 다른 설정 저장으로 출력 선호를 잃지 않도록 한다. 설정창 재개방 시 다시 조회하고 실제 open 실패의 기존 복구는 유지한다. SDL 자동 출력은 드라이버가 이름 목록을 제공하지 않아도 기본 장치를 열 수 있다는 API 계약을 유지한다. 현재 열린 창의 실시간 hotplug 감지·자동 최저지연 backend 선정은 별도 미완료다. 모바일 터치 UI는 장기 후속으로만 기록한다.


1.1.91은 DSi 32/47kHz 전환 때 host resampler의 delta 기준을 보존한다. 기존 SetSampleRate가 직전 제출 값을0으로 지워 일정한 신호에도 추가 DC 단차를 만들었다. 생성 시에만0으로 초기화하고 rate 변경은 기존 제출 값을 유지한다. 같은 rate 반복 설정도 동일하게 처리하며 상태 형식은 변경하지 않는다. AD-08은 private 최종 PCM 연결에서 DS NITRO·DSi DSP 혼합/음소거·10bit·blip SRC까지 전달을 확인했으나 아직 제품 옵션이 아니다. 합성511Hz 한 조건의 image 억제 개선과 source-rate 의존 추가 지연을 함께 기록했다. 계수/옵션 배포, 모든 제어 이력 상한과 실제 게임/장치/ARM 수락이 남는다.


AD-08 진행 중 후보(1.1.91 이후, 미배포): 준비된 bank/stream/renderer를 src에 옮기고 SPU가 unique_ptr로 소유한다. 생성 전 준비 API를 통해 NDS noexcept 생성 경로 안에서 새 bank/renderer를 할당하지 않는다. 이력 예약은 bank support와 Run 동안 고정 reload라는 실제 이벤트 조건에서 유도한다. 이미 예약된 DSi 이벤트는 이전 주기로 실행되므로 실제 cycles에 맞춰 처리한다. 다음은 계수 파일의 재현 가능한 배포/준비와 설정 옵션 연결, 옵션 변경 실패 복구, 실제 게임/장치·ARM 검증이다. 현재 소스/API 연결을 사용자 선택 옵션 출시로 세지 않는다.


1.1.92 — 최소위상 보간의 내장 계수·설정 옵션을 연결했다. 새 renderer 준비는 콘솔 생성 예외를 전달하거나 기존 콘솔 변경 전에 완료하고, 실패하면 이전 mode/config를 유지한다. 이전 후보 문단의 noexcept 생성 경로 설명은 이 변경으로 대체한다. 기본 보간/음높이 유지 설정은 바꾸지 않는다. 실제 Qt 게임에서 미리보기·취소·저장 및 다른 설정 후 유지가 통과했으나 출력 부족은 남는다. 다음은 AD-04의 시작/설정 복귀 공급 부족을 시점별로 분리하고, AD-08의 독립 계수 재생성 도구와 native ARM·청취를 검증한다. 별도로 기존 JIT 하위 생성자의 noexcept 할당 실패가 프로세스를 종료하는 반례를 보존했으며 아직 수정하지 않았다. 새 renderer 할당 실패 통과를 전체 생성자의 OOM 복구로 확대하지 않는다.
...
1.1.96 — 정상 속도/R3 OFF에서도 audio sync가 실제 적용되는 경우 중복 FPS 대기를 제거한다. 비R3 저속/배속에서 audio sync를 생략하던 조건을 같은 판단으로 재사용해 기존 FPS 제한을 보존한다. 사용자 우선순위에 따라 추가 pause/resume 튜닝과 극저속 R3는 보류하고, 낮은 버퍼의 실행 중 안정성 및 비오디오 독립 기능을 병렬로 진행한다. R3 15FPS는 하드웨어 저성능 측정이 아니라 의도적 1/4배속 설정의 호환 문제다.

1.1.97 — 비오디오 독립 블록: LocalMP 호스트별 응답 격리와 역할 전환, 선택형 렌더링 단계 계측, GDB 포트 중복 입력 방어. 계측은 성능 향상 자체가 아니며 실제 게임 2그룹/Qt present/장치별 잔여 수락은 유지한다. 다음은 낮은 버퍼 장기 실행·실제 출력 장치 복구와 계측으로 확인된 비용만 최적화한다. 추가 pause/resume·극저속 R3는 계속 보류한다.

1.1.98 — AD-04 장치 상실 복구: 끊긴 출력 장치에서 SDL IsRunning이 stale로 남던 문제를 NeedsRecovery 기준으로 해소하고, 죽은 핸들은 같은 설정 재적용으로 재오픈한다. UI 500ms 타이머가 저장된 backend/device/버퍼를 재시도하고 실패하면 같은 요청 버퍼의 임시 SDL 기본으로 폴백하며 Config 저장 선호는 덮지 않는다. 설정 대화상자가 열려 있으면 보류하고, 실패는 2배 백오프로 최대5초까지 재시도하며 정상 폴백은 건드리지 않는다. 다음은 물리 분리·재라우팅, 드라이버 Open/Close 지연(복구가 UI 스레드를 동기로 막으며 비블로킹 보장 없음), 장기 청취 확인이다. 추가 pause/resume·극저속 R3는 계속 보류.

1.1.99 — AD-04 SPU 생산 큐의 샘플별 복사를 연속 두 구간 복사로 바꾸고 FIFO/폐기 정책을 유지한다. FS-18은 연결된 중단 debugger를 socket readiness로 대기하며 listener가 닫히면 종료한다. 다음은 UI에서 중단 CPU를 취소하는 소유권·실제 bind 실패 전달과 남은 장치/청취 수락이다. 추가 pause/resume·극저속 R3는 보류한다.

1.1.100 — 펌웨어 메뉴 부팅 가능 여부를 BIOS/JIT/코어·카트 교체 전 검사한다. FreeBIOS/부팅 불가능한 DS 펌웨어 또는 누락된 DSi NAND의 실패가 현재 세션과 대기 카트를 변경하지 않는다. GDB 제어 시제품에서 발견했지만 이 수정은 일반 실행에도 적용되며 시제품 자체의 배포를 뜻하지 않는다. 낮은 버퍼 후속은 전체 callback 비용 측정이며 중단/재개·극저속 R3 보류는 유지한다.

1.1.101 — FS-18은 실제 GDB listener 상태를 조회해 초기화 실패한 ARM 코어와 TCP 포트를 OSD에 알린다(OSD 표시 설정을 따름). 에뮬레이션은 유지하며 정상 listener/JIT 실행에는 실패 알림을 내지 않는다. 제어 스택 시제품과 플랫폼별 전체 수명 수락은 별도 미완료다. 오디오 정상 재생의 간헐적 공급 부족 원인도 미해결이며 1ms 전달 묶음 후보는 이득 미입증으로 미채택이다.

1.1.102 — FS-18/C9 제어 스택을 Qt 제품에 통합했다. GDB 인터프리터만 같은 OS 스레드의 별도 스택을 사용하며 시작/중간 중단에서도 UI·리셋·종료를 처리한다. 준비 실패는 원래 프레임 유지, 성공한 변경은 기존 스택을 먼저 정리한다. 프레임 스텝과 중첩 pause, GL 복귀를 연결했고 명령 중간 스냅샷은 거절한다. 완료된 프레임의 저장/로드/undo 형식은 유지한다. Windows 제품 경로 검증이며 다른 OS 실 실행·GNU GDB 전체 UI·그래픽 생성 실패/송신 포화는 남는다.

1.1.103 — AD-04 자동 출력 복구의 native Open/Close를 장치별 전용 스레드로 옮겼다. 복구 중 게임/UI는 진행하고, 준비된 출력만 생산자를 짧게 멈춰 적용한다. 저장 선호 변경 시 늦은 결과는 재생하지 않고 같은 owner에서 교체한다. 기존 요청 버퍼/선호·임시 SDL fallback·최대5초 백오프를 유지한다. 초기화/수동 적용/종료의 native 대기는 여전히 동기이며 영구 정지 드라이버의 종료 상한, 물리 분리·다른 OS·일반 재생의 간헐적 틱 원인은 남는다.
