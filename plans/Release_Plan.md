...
최근 [실제 1.1.34 증분](releases/1.1.34.md)은 alpha·fog 정밀도, A64/x64 조건부 사이클, LAN host 시각과 배포 source/EXE 결속을 다룬다. [1.1.33](releases/1.1.33.md)의 Compute 작업량 분할과 함께 재현한 독립 결함의 구현 범위를 기록하며, 아래 책임 버전의 Vulkan·다른 플랫폼·실기 수락이 완료됐다는 의미는 아니다. [1차 코드 감사](Audit_2026-09-11.md)는 미검토 범위와 추가 후보를 구분한다.
