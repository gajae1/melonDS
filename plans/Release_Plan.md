...
다음 실제 증분 1.1.18에서는 보간 켬·낮은 출력 버퍼에서 드물게 발생하는 틱 노이즈를 AD-04의 우선 재현 조건으로 다룬다. 코어 작업과 함께 진행할 [5페이즈 착수 계획](Execution_Order.md)을 따르며, 아직 underrun·장치·보간 중 어느 것이 원인인지 확정하지 않았다.
...
AD-08의 보간 비교에 채널별 대역 제한 sinc 후보를 추가하고 다음 실제 증분 1.1.18에서 검토를 시작한다. 현재 최종 출력의 blip_buf와 역할을 구분하며, 음질·연산량·추가 지연을 비교하기 전 새 보간 방식이나 기본값 변경을 확정하지 않는다. 틱 노이즈의 공급·동기화 진단은 AD-04에서 별도로 진행한다.
