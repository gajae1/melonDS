...
현재 제품의 1.1.130 구현·검증은 [릴리스 기록](releases/1.1.130.md)과 [현재 상태](Current_Status.md)를 따른다. 아래 책임 버전과 페이즈의 번호·완료 기준은 유지한다.
...
최근 [실제 1.1.36 증분](releases/1.1.36.md)은 ARM7 x64 JIT의 전체 주소 영역 판별과 LAN 관찰 전용 통계·로비 표시·계측 비용을 다룬다. [1.1.35](releases/1.1.35.md)의 느린 참가자 유지·대기 상한과 Thumb 관측을 이어간다. 아래 책임 버전의 Vulkan·다른 플랫폼·실기 수락이 완료됐다는 의미는 아니다. [1차 코드 감사](Audit_2026-09-11.md)는 미검토 범위와 추가 후보를 구분한다.
...
[1.1.18 증분](releases/1.1.18.md)은 AD-04의 낮은 출력 버퍼에서 생산자가 과도하게 기다리는 공급 경계와 부족/복귀 파형을 다룬다. 실제 SDL 장치 공급 측정과 사용자 틱의 원인 동일성·장기 청취·물리 지연을 구분한다. 장치 재연결·전체 host queue 복구는 계속 남긴다.
...
AD-08은 [1.1.18 증분](releases/1.1.18.md)에서 SpeexDSP Q3를 고정 timer 합성 입력으로 비교했다. 실제 채널의 변속 history·capture·상대 지연·tick 비용·청취를 확인하기 전 제품 통합은 보류한다. 최종 출력 blip_buf와 역할을 구분하고 틱 노이즈의 공급·동기화는 AD-04에서 별도로 다룬다.
...
[실제 1.1.23 증분](releases/1.1.23.md)은 GR-03/04/11의 표시 shader·첫 current 실패, 부분 해제·OSD 재생성과 전체 Qt 두 창의 native 전환·명시적 재선택을 다룬다. 실행 중 current/swap 실패·해제 거부와 실제 surface 복구는 다음 증분이며, 책임 버전 1.1.23의 DSi·카메라 과제를 완료 처리하지 않는다.
