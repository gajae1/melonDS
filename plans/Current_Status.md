...
현재 변경의 범위·실제 검증은 [1.1.143 기록](releases/1.1.143.md)을 따른다.
...
| 캐시/전송 | RAM-only cache·비우기·32MiB 소프트 제한, clear 묶음 전송·cached readback; 1.1.143 프레임 단위 업로드 묶음 제출(texture·clear·capture를 phase당 한 번) | GPU 직접 표시; 묶음의 실게임 wall-time 효과 미측정 |
...
4. texture 업로드 묶음은 1.1.143이 프레임 phase당 한 번 제출로 줄였다(20프레임 fixture 582→37회). GPU 2D·직접 표시를 작은 단계로 진행하고 scanline/FIFO/capture 가시성·복구 수명을 보존한다.
