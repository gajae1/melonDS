...
기준 커밋은 1.1.134 `7fdb93c6`. 현재 변경의 범위·실제 검증은 [1.1.135 기록](releases/1.1.135.md)을 따른다.
...
연속 행 샘플링의 추가 최적화는 도구 차단으로 제품에 넣지 않았다. 계획과 미적용 후보는 실행 증거와 구분한다.

## 다음 단계

1. 비트맵 합성의 CPU 병목을 줄인다. 연속 행 샘플링 후보는 byte 비교·경계 검사·실측 뒤에만 채택한다.
