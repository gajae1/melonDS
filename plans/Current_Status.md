...
2026-09-26. 코드와 빌드의 기준 경로는 `F:/melonDS`다. [1.1.186 릴리스 기록](releases/1.1.186.md)에 소스 커밋·패키지 해시·검증 범위를 기록한다. SDL3·SDL2 패키지와 Windows·Ubuntu·macOS·BSD·Core regressions CI가 모두 통과했다.
...
| CPU/JIT | x64·ARM64 정적 분기 블록 연결을 유지한다. 1.1.184는 가장 이른 이벤트 시각을 캐시하고 ARM9 느린 메모리 접근에서 CPU 소유 NDS 참조를 사용한다. | ARM64 실장치 속도는 아직 측정하지 않았다. ARM7 소유 참조 변경은 측정 이득이 기준에 못 미쳐 채택하지 않았다. |
| 소프트웨어 렌더러 | 1.1.183에서 저장 상태 복원 뒤 남은 스캔라인 신호 때문에 오래된 3D 줄을 읽을 수 있던 경쟁 상태를 수정했다. 1.1.185는 GPU2D 텍스트 타일 행 조회·native 픽셀 저장·불필요한 캡처 작업을 줄였다. | 모든 장면의 속도 향상을 입증한 것은 아니다. 2코어에서 느려진 소프트웨어 3D 병렬화 후보는 채택하지 않았다. |
| 플랫폼/배포 | Windows 기본 배포는 SDL3, SDL2는 별도 호환 ZIP이다. 두 1.1.186 패키지는 같은 소스 식별값을 사용하고 각 325개 파일·비공개 파일 0개를 확인했다. | SDL3 사용 자체가 코어 FPS 향상을 뜻하지 않는다. 런타임/소스 ZIP 및 검증 범위는 버전별 기록을 따른다. |

## 성능 수치의 범위

- 원본과 직접 비교한 가장 최근 기록은 **1.1.180 대 포크 기준 원본**이다. Ryzen 7 9800X3D/Windows, 소프트웨어 렌더러, 두 게임의 시작 1200프레임에서 실행 시간은 JIT+fastmem 기준 Solatorobo 약 3.5%, Black 약 10.1% 줄었다. 인터프리터는 각각 약 3.9%, 5.7%였다. 같은 바이너리 A/A 대조를 포함한다. 다만 Black/JIT은 마지막 화면의 진행도가 다르고, 오디오 해시도 포크의 정확도 변경으로 원본과 다르므로 완전히 동일한 연산의 속도 비교는 아니다. 다른 장면·저사양 기기·GPU 렌더러·1.1.186 전체의 향상률로 확대하지 않는다. 근거: `local-docs/upstream-speed-20260925/result.md`.
...
- 1.1.185 GPU2D 변경은 같은 두 게임의 Linux 300프레임, fastmem OFF 측정에서 실행 명령어 수를 2.546%, 1.377% 줄였다. Windows 시간 비교는 반복 변동보다 작은 차이여서 FPS 향상률로 제시하지 않는다. 실제 저사양 기기의 전체 장면을 측정한 결과는 아니다.

- 1.1.186은 32개 텍스처와 clear bitmap을 갱신하는 20프레임에서 GPU 제출이 각 배율당 37회에서 19회로 줄었다. 1배율·3배율의 native 화면은 독립 소프트웨어 렌더러와 바이트 단위로 일치했다. 업로드가 드문 장면의 이득은 작을 수 있으며 FPS 향상률은 측정하지 않았다.
