...
from unicorn.arm64_const import (UC_ARM64_REG_X0, UC_ARM64_REG_X4,
    UC_ARM64_REG_X19, UC_ARM64_REG_X20, UC_ARM64_REG_X29, UC_ARM64_REG_X30,
    UC_ARM64_REG_W27, UC_ARM64_REG_W28, UC_ARM64_REG_PC)

export = subprocess.run([sys.argv[1], "export-mpu"], capture_output=True, text=True, timeout=30)
assert export.returncode == 0, export.stderr
cases = [json.loads(line) for line in export.stdout.splitlines() if line.startswith("{")]
assert len(cases) == 8
checked = 0
for case in cases:
    for permission in (0, 1, 2, 3):
        for address in (0x02001000, 0xFFFFFFFF):
            ...
            denied = case["num"] == 0 and not (permission & (2 if case["store"] else 1))
            ...
            if denied:
                ...
                assert get32(regs + 16) == address and get32(regs + 20) == 0xCAFEBABE
