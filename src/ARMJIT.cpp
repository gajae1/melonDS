...
// Fixed scratch capacity; the configured limit still controls tracing.
static constexpr unsigned MaxBlockSizeLimit = 32;

ARMJIT::ARMJIT(melonDS::NDS& nds, std::optional<JITArgs> jit) noexcept : 
        NDS(nds),
        Memory(nds),
        JITCompiler(nds),
        MaxBlockSize(jit.has_value() ? std::clamp(jit->MaxBlockSize, 1u, MaxBlockSizeLimit) : MaxBlockSizeLimit),
...
void ARMJIT::SetJITArgs(JITArgs args) noexcept
{
    ...
    args.MaxBlockSize = std::clamp(args.MaxBlockSize, 1u, MaxBlockSizeLimit);
...
void ARMJIT::CompileBlock(ARM* cpu) noexcept
{
    ...
    u32 addressRanges[MaxBlockSizeLimit];
    ...
    u32 literalValues[MaxBlockSizeLimit];
    u32 instrValues[MaxBlockSizeLimit];
    // due to instruction merging i might not reflect the amount of actual instructions
    u32 numInstrs = 0;

    u32 writeAddrs[MaxBlockSizeLimit];
...
void ARMJIT::InvalidateByAddr(u32 localAddr) noexcept
{
    ...
    const u32 writeMask = 1u << ((localAddr & 0x1FF) / 16);
